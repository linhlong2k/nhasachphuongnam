...
public class OrderController {
	...
	public String createGET(ModelMap model) {
		...
		return "orders/createAndUpdate";
	}
	@RequestMapping(value="create", method=RequestMethod.POST)
	public String createPost(ModelMap model,
			@ModelAttribute("order") Order order) {
		//create order lên datebase
			//thành công
				//thêm message thông báo kết quả
				//trả về views index
			//thất bại
				//thông báo không thành công
				//trả về views createAndUpdate
		return "orders/index";
	}
	@RequestMapping(value="update/{id}", method=RequestMethod.GET)
	public String updateGET(ModelMap model,
			@PathVariable("id") String maDonHang) {
		Order order = new Order();//get order by ma Don hang
